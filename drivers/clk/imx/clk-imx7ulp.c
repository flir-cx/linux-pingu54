...
static void __init imx7ulp_clk_pcc3_init(struct device_node *np)
{
	...
	struct clk_hw **hws;
	void __iomem *base;
	static u32 fake_mipi_dsi_reg;
	static u32 fake_lcdif_reg;
	...
	hws[IMX7ULP_CLK_DSI]		= imx7ulp_clk_hw_composite("dsi",     periph_bus_sels, ARRAY_SIZE(periph_bus_sels), true, true,  true, &fake_mipi_dsi_reg); // base + 0xa4);
	hws[IMX7ULP_CLK_LCDIF]		= imx7ulp_clk_hw_composite("lcdif",   periph_plat_sels, ARRAY_SIZE(periph_plat_sels), true, true,  true, &fake_lcdif_reg); //base + 0xa8);

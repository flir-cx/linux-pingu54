...
#if 0  // Unsupported Android feature
...
#define MX6UL_IOMUXC_BASE_ADDR       0x020e0000
#define MX6UL_IOMUXC_PAD_RXER_OFFSET 0x5c4         // IOMUXC_SW_PAD_CTL_PAD_ENET_RX_ER
#define PULLDOWN_IDPIN 0xffff3fff                  // 00 = 100k Ohm Pull Down
#define PULLUP_IDPIN   (1<<14)                     // 01 = 47k Ohm Pull Up

#include "fusb30x_global.h"                                                     // Chip structure access
#include "../core/core.h"                                                       // Core access
#include "../core/vendor_info.h"
#include "../core/vdm/vdm.h"
...
FSC_S32 fusb_InitializeGPIO(void)
{
    ...
    {
        // Soft fail - provide a warning, but don't quit because we don't really need debug
        pr_info("%s - Warning: Could not get GPIO for Debug GPIO!\n", __func__);
    }
    else
    {
        // Request our GPIO to reserve it in the system - this should help ensure we have exclusive access (not guaranteed)
...
FSC_S32 fusb_InitializeUSBMux(void)
{
    FSC_S32 ret = 0;
    struct device_node* node;
    struct fusb30x_chip* chip = fusb30x_GetChip();
...
void fusb_GPIO_Set_OTG(FSC_BOOL set)
{
    static FSC_BOOL old_set = FALSE;
    void *addr;
    unsigned int tmp;

    addr = ioremap(MX6UL_IOMUXC_BASE_ADDR+MX6UL_IOMUXC_PAD_RXER_OFFSET, 1);
    tmp = ioread32(addr);
    if (set) {
        if (set != old_set)
            pr_info("%s: Enable OTG\n",__func__);
        iowrite32(tmp & PULLDOWN_IDPIN,addr);
    } else {
        if (set != old_set)
            pr_info("%s: Disable OTG\n",__func__);
        iowrite32(tmp | PULLUP_IDPIN, addr);
    }
    old_set = set;
    tmp = ioread32(addr);
    iounmap(addr);
...
void fusb_GPIO_Set_VBus5v(FSC_BOOL set)
{
    ...
    if (set) {
        if (set != old_set)
            pr_info("%s: Enable VBUS 5V\n",__func__);
        bq24298_enable_otg();
    } else {
        if (set != old_set)
            pr_info("%s: Disable VBUS 5V\n",__func__);
        bq24298_disable_otg();
    }
    old_set = set;
...
/*******************************************************************************
* Function:        _fusb_Sysfs_set_pid_store
* Input:           buf: Data written (new mode as zero terminated string)
*                  count: Number of chars in buf
* Return:          Number of chars read from buf
* Description:     Sets what USB Product ID to report through USB PD.
********************************************************************************/
static ssize_t _fusb_Sysfs_set_pid_store(struct device* dev,
		struct device_attribute* attr, const char* buf, size_t count)
{
    extern FSC_U16 usb_product_id;   /* from vdm_callbacks.c */
    int err = 0;

    if (!buf || !count)
    {
        pr_err("%s - Error: Input buffer is NULL or empty!\n", __func__);
        err = -EFAULT;
    }
    else
    {
        if (!memcmp(buf, "MSD\n", 4))
            usb_product_id = USB_PID_FLIR_MSD;
        else if (!memcmp(buf, "RNDIS\n", 6))
            usb_product_id = USB_PID_FLIR_EX_RNDIS;
        else if (!memcmp(buf, "UVC\n", 4))
            usb_product_id = USB_PID_FLIR_EX_UVC;
        else if (!memcmp(buf, "RNDIS_UVC\n", 10))
            usb_product_id = USB_PID_FLIR_EX_RNDIS_UVC;
        else if (!memcmp(buf, "RNDIS_MSD\n", 10))
            usb_product_id = USB_PID_FLIR_EX_RNDIS_MSD;
        else if (!memcmp(buf, "UVC_MSD\n", 8))
            usb_product_id = USB_PID_FLIR_EX_UVC_MSD;
        else if (!memcmp(buf, "ZERO\n", 5))
...
void fusb_update_conn_state(ConnectionState conn_state)
{
    ...
    {
        case AttachedSink:
            new_state_str = "powered\n";
            break;
        case AttachedSource:
            new_state_str = "attached\n";
            break;
        case IllegalCable:
        case IllegalSource:
            new_state_str = "failure\n";
            break;
        default:
            new_state_str = "not attached\n";
            break;
    }

    if (new_state_str != conn_state_str)
    {
        struct fusb30x_chip* chip = fusb30x_GetChip();
        conn_state_str = new_state_str;
        sysfs_notify(&chip->client->dev.kobj, "control", "state");
    }
}

static ssize_t _fusb_Sysfs_State_show(struct device* dev, struct device_attribute* attr, char* buf)
{
    strcpy(buf, conn_state_str);
    return strlen(conn_state_str);
}

static DEVICE_ATTR(set_pid, S_IWUSR, NULL, _fusb_Sysfs_set_pid_store);
static DEVICE_ATTR(state, S_IRUGO, _fusb_Sysfs_State_show, NULL);
...
void fusb_Sysfs_Init(void)
{
    ...
    fusb_update_conn_state(Disabled);
}
...
FSC_S32 fusb_EnableInterrupts(void)
{
    ...
    //Don't enable_irq_wake(chip->gpio_IntN_irq);
...
static irqreturn_t _fusb_isr_intn(FSC_S32 irq, void *dev_id)
{
    ...
    //printk("FUSB Enter IRQ (%u)\n", platform_get_device_irq_state());
    ...
    //printk("FUSB Exit IRQ\n");

    return IRQ_HANDLED;
}

void fusb_platform_suspend(void)
{
    struct fusb30x_chip* chip = fusb30x_GetChip();

    pr_info("FUSB: suspend\n");
    ...
    */
    ...
    pr_info("FUSB: suspended\n");
}

void fusb_platform_resume(void)
{
    struct fusb30x_chip* chip = fusb30x_GetChip();

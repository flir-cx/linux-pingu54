...
{
    static FSC_BOOL remainingData;

    if (PDTxStatus == txReset)                                                  // If we need to send a hard reset...
    {
        ProtocolState = PRLReset;                                               // Set the protocol state to send it
    }
    else if (Registers.Status.I_GCRCSENT || remainingData)                      // Otherwise check to see if we have received a message and sent a GoodCRC in response
    {       
        if(!g_collision_lock)
        {
            ProtocolFlushTxFIFO();
        }
        remainingData = ProtocolGetRxPacket();                                  // Grab the received message to pass up to the policy engine
    ...
    {
        if (PolicyTxHeader.MessageType == DMTSourceCapabilities)		// Flush RX fifo before starting new session
            ProtocolFlushRxFIFO();
...
void ScanRxFifoForSop(FSC_U8 *data)
{
    /* Read 3 bytes from RX fifo (SOP + header bytes).
     * If first byte is not SOP* then drop bytes until one is found
     * NOTE: According to FUSB30x user manual, the low 5 bits of a SOP*
     *       could have any value but in practice they are always zero (?).
     */
    FSC_U32 i, j;
    FSC_U32 loops = 0;
    ...
    for (;;) {
        ...
        i = 0;
        while ((i < 3) && ((data[i] & 0x1f) || (data[i] < 0xA0)))
            ++i;
        ...
        // Some good, move them down
        j = 0;
        while (i < 3)
            data[j++] = data[i++];

        // Fill up from FIFO
        DeviceRead(regFIFO, (3 - j), &data[j]);
        return;
    }
}

FSC_BOOL ProtocolGetRxPacket(void)
{
    FSC_U32 i, j;
    FSC_U8 data[3];
    SopType rx_sop;
    ...
    ScanRxFifoForSop(data);                                                     // Read the Rx token and two header bytes
    ...
    */
    ...
    // Check if more messages are available in FIFO (CRC_CHK and not RX_EMPTY)
    DeviceRead(regStatus0, 2, &data[0]);
    return ((data[0] & 0x10) && !(data[1] & 0x20)) ? TRUE : FALSE;
}

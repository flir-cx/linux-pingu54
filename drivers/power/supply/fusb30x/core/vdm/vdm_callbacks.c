...
FSC_U16 usb_product_id;  /* Set from platform_helpers.c by sysfs write call */
...
Identity vdmRequestIdentityInfo() {
    ...
    if(Responds_To_Discov_SOP) {
        ...
        id.product_vdo.usb_product_id = usb_product_id;
    ...
    if((modes_info.svid == SVID_AUTO_ENTRY) && (modes_info.nack == FALSE)) {
        for (i = 0; i < modes_info.num_modes; i++) {
            if (evaluateModeEntry(modes_info.modes[i])) {
                ...
                AutoModeEntrySVID = SVID_AUTO_ENTRY;
            }
        }
    }
    else if (modes_info.svid == DP_SID && modes_info.nack == FALSE) {
        for (i = 0; i < modes_info.num_modes; i++) {
            if (platform_evaluate_dp_mode(modes_info.modes[i])) {
                AutoModeEntryObjPos = i+1;
                AutoModeEntrySVID = DP_SID;
            }
        }
    }
#endif // FSC_HAVE_DP
...
void vdmInformAttention(FSC_U16 svid, FSC_U8 mode_index, FSC_U32 vdo)
{
    if (svid == DP_SID) {
        DisplayPortStatus_t peer_status;

        peer_status.word = vdo;
        informStatus(peer_status);
    }
}

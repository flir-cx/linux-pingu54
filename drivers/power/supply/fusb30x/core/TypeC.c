...
TIMER                 FailureTimer;           // Timer used to abort when attachment makes no sense
...
//    DeviceWrite(regSwitches1, 1, &Registers.Switches.byte[1]);
    ...
    DetachThreshold = VBUS_THRESH_DETACH_5V;    // Default to 5V detach threshold
    ...
    platform_set_timer(&FailureTimer, T_TIMER_DISABLE);           // Disable the failure timer
...
void StateMachineTypeC(void)
{
        ...
        {        
            /* UGLY FIX!
             * Some dongles have excessive inrush current on VCONN that will
             * trigger the FUSB over current protection. Allow this by clearing
             * the OCP condition through toggling the VCONN output off/on.
             */
            unsigned old_ocp = Registers.Status.OCP;
            ...
            if (Registers.Status.OCP) {
                if (!old_ocp) {
                    if (platform_restart_vconn_allowed()) {
                        FSC_U8 sw0 = Registers.Switches.byte[0] & 0xcf;  // Mask out VCONN_CC1 and VCONN_CC2
                        DeviceWrite(regSwitches0, 1, &sw0);
        ...
        platform_update_current_limit();
        platform_update_conn_state();
...
{
    if (platform_check_timer(&FailureTimer))
    {
        // Just give up and wait for unattach
        SetStateIllegalSource();
        return;
    }
...
//    }
    ...
    else if (isVBUSOverVoltage(VBUS_THRESH_ATTACH_5V))                                           // If we have detected VBUS and we have detected an Rp for >tCCDebounce...
    {
...
void StateMachineTrySource(void)
{
    ...
    else if ((platform_check_timer(&StateTimer)) && (CCTermPDDebounce <= CCTypeRa))             // If we haven't detected Rd
    {
...
void SetStateAttachWaitSink(void)
{
    ...
    platform_set_timer(&FailureTimer, tFailureTimeOut);
}
...
void SetStateUnattachedSource(void) // Currently only implemented for transitioning from AttachWaitSnk to Unattached for DRP
{
    ...
    // Reenable automatic TOGGLING of CC pull resistors and measurements
...
{
    return !isVBUSOverVoltage(VBUS_THRESH_SAFE_0V);
}

/* UGLY FIX!
 * This function replaces VbusVSafe0V() in some cases.
 * Some tested USB-C to HDMI dongles are leaking voltage from HDMI I2C
 * to VBUS making this test fail. Changing limit to 3.36V making at least
 * the tested dongle work.
 * Uncertain what trouble this may cause for other types of dongles
 * and/or error detection.
 */
FSC_BOOL VbusVSafeSink(void)
{
    return !isVBUSOverVoltage(VBUS_THRESH_SAFE_SINK);
}

FSC_BOOL isVSafe5V(void)                                                            // Returns true when Vbus > ~4.6V
{
    return isVBUSOverVoltage(VBUS_THRESH_ATTACH_5V);
}
...
void debounceCC(void)
{
    ...
    else if (platform_check_timer(&PDDebounceTimer))                            // Check to see if our debounce timer has expired...
    {
    ...
    else if (platform_check_timer(&CCDebounceTimer))
    {
    ...
    //printk("FUSB: CCTermPDDebounce=%d, CCTermCCDebounce=%d\n",CCTermPDDebounce,CCTermCCDebounce);
}
...
void SetStateIllegalSource(void)
{
#ifdef FSC_INTERRUPT_TRIGGERED
    g_Idle = TRUE;
    platform_enable_timer(FALSE);
    ...
    loopCounter = 0;
    ConnState = IllegalSource;
    setStateSink();
    UpdateSinkCurrent();
    platform_set_timer(&StateTimer, T_TIMER_DISABLE);
}

void StateMachineIllegalSource(void)
{
    if (VbusVSafe0V())    // Require complete disconnect
    {
        SetStateDelayUnattached();
    }
}

...
extern FSC_U16                  AutoModeEntrySVID;
#endif // FSC_HAVE_DP
...
#ifdef FSC_HAVE_SNK
    SinkRequestMaxVoltage = PD_MaxVoltage_as_Sink;                              // Maximum voltage that the sink will request
    SinkRequestMaxPower = PD_MaxPower_as_Sink;                                  // Maximum power the sink will request
    SinkRequestOpPower = PD_OpPower_as_Sink;                                    // Operating power the sink will request
...
/* This function is called after Hard Reset or detach */
#ifdef FSC_HAVE_VDM
void ResetVDMVariables(void)
{
    int i;
...
void PolicySourceStartup(void)
{
    ...
    switch (PolicySubIndex)
    {
        case 0:
            ...
            platform_set_timer(&PolicyStateTimer, tVBUSSourceOn);
            PolicySubIndex++;
            break;
        case 1:
            if ((isVBUSOverVoltage(VBUS_THRESH_SOURCE_MIN_5V) || (ConnState == PoweredAccessory)) && (platform_check_timer(&SwapSourceStartTimer)))                    // Wait until we reach 5V source thresh and delay if coming from PR Swap
            {
                PolicySubIndex++;
            }
            else if (platform_check_timer(&PolicyStateTimer))                   // No VBUS generated
            {
...
void PolicySourceTransitionSupply(void)
{
    ...
    switch (PolicySubIndex)
    {
        ...
        case 5:
            if(CapsSource[USBPDContract.FVRDO.ObjectPosition-1].FPDOSupply.Voltage == 100)
            {
                if((!isVBUSOverVoltage(VBUS_THRESH_SOURCE_MAX_5V) && isVBUSOverVoltage(VBUS_THRESH_SOURCE_MIN_5V)) || (platform_check_timer(&PolicyStateTimer)))   // Check that VBUS is between 4.6 and 5.5 V
                {
                    DetachThreshold = VBUS_THRESH_DETACH_5V;   // 5V detach threshold
                    PolicySubIndex++;
                }
            }
#ifdef FM150911A
...
void PolicySourceReady(void)
{
    ...
    else if(PartnerCaps.object == 0)
    {
        ...
        PDTxStatus = txIdle;                                                    // Clear the transmitter status
            ...
            (PartnerCaps.FPDOSink.DualRolePower == 1) &&
            (PartnerCaps.FPDOSink.ExternallyPowered == 1))
    {
        PolicyState = peSourceSendPRSwap;                       // Issue a PR_Swap message
        PolicySubIndex = 0;                                     // Clear the sub index
        PDTxStatus = txIdle;                                    // Clear the transmitter status        
    ...
    switch(PolicySubIndex)
    {
        case 0: // Send either the Accept or Reject command
            if (!Accepts_PR_Swap_As_Src || (UUT_Device_Type == 3) ||
                    /* UGLY FIX!
                     * The Apple dongle first states (in SINK CAP message) that it does not support
                     * power role swap but then still goes ahead and requests a swap if it has
                     * external power attached.
                     * Ignore the inconsistency and let the sink do a swap if it requests one.
                     */
                    //((CapsSource[0].FPDOSupply.DualRolePower == FALSE) || (PartnerCaps.FPDOSink.DualRolePower == FALSE) || // Determine Accept/Reject based on DualRolePo
                    ((CapsSource[0].FPDOSupply.DualRolePower == FALSE) ||
...
void PolicySinkTransitionDefault(void)
{
    switch (PolicySubIndex)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            if(isVBUSOverVoltage(VBUS_THRESH_ATTACH_5V))
            {
                PolicySubIndex++;
            }
            else if (platform_check_timer(&PolicyStateTimer))                                      // Break out if we never see VBUS
            {
                if(PolicyHasContract)
                {
                    PolicyState = peErrorRecovery;
...
void PolicySinkSendPRSwap(void)
{
    ...
    switch(PolicySubIndex)
    {
        ...
        default: // Allow time for the supply to rise and then send the PS_RDY message
            if (isVBUSOverVoltage(VBUS_THRESH_SOURCE_MIN_5V))
            {
        ...
        switch (AutoVdmState) {
            ...
            case AUTO_VDM_ENTER_MODE_PP:
                if (AutoModeEntryObjPos > 0) {
                    requestEnterMode(SOP_TYPE_SOP, AutoModeEntrySVID, AutoModeEntryObjPos);
...
void GetContract(FSC_U8* abytData)
{
    FSC_U32 j, i;
    if (PolicyHasContract) {
        *abytData++ = 1;
        *abytData++ = CapsHeaderReceived.byte[0];
        *abytData++ = CapsHeaderReceived.byte[1];

        i = ((USBPDContract.object >>28) &0x7)-1;

        for (j=0;j<4;j++) {
            *abytData++ = CapsReceived[i].byte[j];
        }
    } else {
        *abytData++ = 0x0;
    }

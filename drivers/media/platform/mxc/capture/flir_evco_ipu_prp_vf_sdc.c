...
static irqreturn_t flir_evco_prpvf_rot_eof_callback(int irq, void *dev_id)
{
	cam_data *cam = dev_id;
...
static int flir_evco_prpvf_start(void *private)
{
	...
	if (cam->vf_rotation >= IPU_ROTATE_VERT_FLIP) {
		...
		err = ipu_request_irq(cam->ipu, IPU_IRQ_PRP_VF_ROT_OUT_EOF,
				      flir_evco_prpvf_rot_eof_callback,
			      0, "Mxc Camera", cam);
		if (err < 0) {
...
EXPORT_SYMBOL(flir_evco_prp_vf_sdc_select);
...
EXPORT_SYMBOL(flir_evco_prp_vf_sdc_deselect);
 ...
 */
__init int flir_evco_prp_vf_sdc_init(void)
{
	return 0;
}
 ...
 * @return  Error code indicating success or failure
 */
void __exit flir_evco_prp_vf_sdc_exit(void)
{
}

module_init(flir_evco_prp_vf_sdc_init);
module_exit(flir_evco_prp_vf_sdc_exit);

MODULE_AUTHOR("Freescale Semiconductor, Inc.");
MODULE_AUTHOR("FLIR.");
MODULE_DESCRIPTION("FLIR EVCO IPU PRP VF SDC Driver");
MODULE_LICENSE("GPL");
